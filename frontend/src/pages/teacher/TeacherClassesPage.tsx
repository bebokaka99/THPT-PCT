...
export function TeacherClassesPage() {
  ...
  const classroomsQuery = useQuery({
    queryKey: ['teacher', 'classrooms', user?.id, query],
  ...
  return (
    <TeacherPortalLayout>
      <header className="border-b border-slate-200 pb-6">
        <p className="text-sm font-semibold text-emerald-700">Không gian giảng dạy</p>
        <h1 className="mt-1 text-2xl font-bold text-slate-950 sm:text-3xl">
          Lớp học phụ trách
        </h1>
        <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">
          Quản lý thông báo, tài liệu và thời khóa biểu của các lớp được nhà
          trường phân công.
        </p>
      </header>
      ...
      {!classroomsQuery.isLoading && !classroomsQuery.isError && classrooms.length === 0 && (
        <div className="mt-6 border border-dashed border-slate-300 bg-white px-6 py-12 text-center">
          ...
          <h2 className="mt-3 font-bold text-slate-900">
            {query ? 'Không tìm thấy lớp phù hợp' : 'Chưa có lớp học được gán'}
          </h2>
      ...
      <section className="mt-6 grid gap-4 md:grid-cols-2">
        {classrooms.map((classroom) => (
          <article
            key={classroom.id}
            className="group border border-slate-200 bg-white p-5 shadow-sm transition hover:border-emerald-300 hover:shadow-md"
          >
            <div className="flex items-start justify-between gap-4">
              <span className="inline-flex h-11 w-11 items-center justify-center rounded-md bg-emerald-50 text-emerald-700">
